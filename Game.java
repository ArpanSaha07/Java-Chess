...
public class Game {
    ...
    public boolean canMove(int x, int y, int destX, int destY, Side s){
       ...
       - piece must move "through" a piece to get from (x,y) to (destX,destY) (use isVisible())  **
        ...
        else if (destX < 0 || destY < 0 ||destX > 7 || destY > 7) {
            return false;
        }
        ...
        else if (b.get(destX, destY)!= null) {
    ...
    public boolean move(int x, int y, int destX, int destY){
        ...
        if (b.getKing(Side.negate(currentTurn)) == null || b.getKing(currentTurn) == null) {
            return false;
        }
    ...
    public boolean isInCheck(Side side) {
        ...
        if (b.getKing(side) != null) {
            ...
            if (!b.getPieces(Side.negate(side)).isEmpty()) {
                for (int i = 0; i < b.getPieces(Side.negate(side)).size(); i++) {
                    if (canMove(b.getPieces(Side.negate(side)).get(i).x, b.getPieces(Side.negate(side)).get(i).y, kingX, kingY, Side.negate(side))) {
                        return true;
                    }
                }
            }
